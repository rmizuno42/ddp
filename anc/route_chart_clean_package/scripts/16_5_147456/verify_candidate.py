"""(16,5) construction-certificate verifier.

Re-checks, from scratch and with no external input, the two facts the (16,5)
route-chart construction relies on:
  ...
  * Fiber chart -- over GF(4)^5, for every reduced word of length 5 in the
    symbols {a, a^-1, b, b^-1} the associated 5x5 controllability matrix is
    nonsingular (full rank 5).

Together these give the lifted graph V(controller) x GF(4)^5 degree 4*4 = 16
and diameter <= 5. (Same controller as the (12,5) case; only the fiber field
changes from GF(3) to GF(4).)

Run as a script to print the OK report. The helper functions and chart data
(build_controller, euler_2factor_coloring, orient_two_factors, A, B, MUL) are
imported by generate_final_graph.py, so their behavior must not change.
"""
...
MOD4, MOD6, NBASE, S = 4, 6, 6, 5    # C4 x C6 factors; 6 base vertices; walk length 5
...
def gf_mul(x, y):
    ...
    Elements are 2-bit integers x = x0 + x1*alpha (0, 1, 2 = alpha, 3 = alpha+1)
    with the rule alpha^2 = alpha + 1.
    """
    x0, x1 = x & 1, (x >> 1) & 1
    y0, y1 = y & 1, (y >> 1) & 1
...
MUL = [[gf_mul(i, j) for j in range(4)] for i in range(4)]   # GF(4) multiplication table
INV = [None, 1, 3, 2]                                        # multiplicative inverses in GF(4)
...
A_HARDCODED = [
    [[2, 0, 1, 2, 0], [1, 2, 2, 0, 0], [3, 0, 0, 3, 0], [0, 0, 1, 3, 0], [2, 1, 2, 2, 2]],  # a
    [[2, 0, 3, 2, 0], [0, 3, 1, 1, 0], [1, 0, 3, 0, 0], [2, 0, 1, 2, 0], [1, 2, 2, 3, 3]],  # a^-1
    [[3, 1, 3, 2, 0], [0, 1, 0, 0, 0], [1, 0, 2, 0, 0], [2, 1, 0, 0, 0], [3, 2, 2, 0, 2]],  # b
    [[0, 3, 0, 3, 0], [0, 1, 0, 0, 0], [0, 2, 3, 2, 0], [3, 1, 1, 2, 0], [0, 2, 3, 3, 3]],  # b^-1
]
B_HARDCODED = [[2, 1, 2, 0, 2], [2, 1, 3, 1, 2], [2, 1, 0, 2, 2], [2, 1, 1, 3, 2]]
...
def matmul(X, Y):
    """Matrix product over GF(4) (entries combined with MUL, summed with XOR)."""
    return [[_dot_row_col(X[i], Y, j) for j in range(len(Y[0]))]
            for i in range(len(X))]


def _dot_row_col(row, Y, j):
    """Dot product of `row` with column j of Y over GF(4)."""
    z = 0
...
def rank_cols(cols):
    """Rank over GF(4) of the matrix whose columns are the vectors in `cols`,
    via Gaussian elimination (XOR is addition, MUL is multiplication)."""
    M = [list(r) for r in zip(*cols)]            # transpose: column list -> row list
    r = 0
    for c in range(len(M[0])):
        ...
        if p is None:
            continue
        M[r], M[p] = M[p], M[r]
        inv = INV[M[r][c]]                       # scale the pivot row to make the pivot 1
        M[r] = [MUL[inv][x] for x in M[r]]
        # Eliminate column c from every other row.
        for i in range(len(M)):
            if i != r and M[i][c]:
                f = M[i][c]
                M[i] = [M[i][j] ^ MUL[f][M[r][j]] for j in range(len(M[0]))]
        r += 1
        if r == len(M):
            break
    return r
...
def verify_chart():
    """Assert the GF(4)^5 chart is a valid universal route chart:
      ...
      * for every length-5 reduced word, the 5 controllability columns are
        linearly independent (rank 5), so the length-5 route reaches all 4^5
        fiber targets.
    ...
    for p, q in [(0, 1), (2, 3)]:
        assert matmul(A[p], A[q]) == I and matmul(A[q], A[p]) == I
    ...
    for w in words:
        ...
        suff = [[1 if i == j else 0 for j in range(5)] for i in range(5)]
        cols = []
        for s in reversed(w):
            cols.append(mv(suff, B[s]))
            suff = matmul(suff, A[s])
        r = rank_cols(cols)
...
if __name__ == '__main__':
    ...
    print(f'GF(4)^5 universal chart: OK; {nwords} reduced words checked, min rank {min_rank}; inverse-symbol data verified')
    print('Certificate complete: degree 16, diameter <=5, vertices = 144*4^5 = 147456')
