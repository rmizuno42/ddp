...
% Make displayed block quotes stand out: extra vertical space + italic.
\BeforeBeginEnvironment{quote}{\vspace{0.7\baselineskip}}
\AfterEndEnvironment{quote}{\vspace{0.7\baselineskip}}
\AtBeginEnvironment{quote}{\itshape}
\usepackage{bookmark}
\IfFileExists{xurl.sty}{\usepackage{xurl}}{} % add URL line breaks if available
\urlstyle{same}
\hypersetup{
  pdftitle={New lower bounds for the degree/diameter problem via interaction with a browser-accessible LLM},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={blue},
...
\title{New lower bounds for the degree/diameter problem via interaction
with a browser-accessible LLM}
...
route-chart lift construction discovered through interaction with
...
and \(132{,}496\). The search was conducted through ordinary dialogue in
the standard ChatGPT web interface, without a dedicated agentic system,
external orchestration, problem-specific search engine, or formal proof
assistant set up in advance by the author. As far as the visible
transcript shows, the author did not prompt the model with the concrete
components of the construction, nor with a problem-specific proof or
construction strategy.

After presenting the mathematical result, we describe the discovery
process on the basis of the visible transcript. We focus on the general,
problem-independent prompts and meta-level interventions under which the
approximately six-day search proceeded, and we identify the stage at
which the abstraction underlying the construction first appeared.

\textbf{MSC 2020:} 05C12, 05C25, 05C76.\\
...
FunSearch {[}RomeraParedes2024{]}, which obtains mathematical
constructions by combining a pretrained LLM with a systematic evaluator
in a program-search loop, and AlphaGeometry {[}Trinh2024{]}, which
combines a neural language model with a symbolic deduction engine for
olympiad geometry. Other examples include AlphaProof {[}Hubert2026{]}, a
reinforcement-learning approach to formal proof search using Lean as a
verifiable environment, and TongGeometry {[}Zhang2026{]}, which
...
AI/LLMs in mathematical discovery. These reports include examples of
counterexample discovery, lower-bound improvement, construction search,
and formal-verification support {[}Bubeck2025, Woodruff2026,
...
long dialogue with ChatGPT in its standard web interface, under general,
problem-independent prompts and meta-level interventions.
...
studied {[}HoffmanSingleton1960, Damerell1973, Bannai1973,
...
for fixed parameters {[}BermondDelormeFarhi1984,
...
The main text first states the theorem, compares it with the known lower
...
\subsection{1.1 Contributions of this
...
\item
  ...
  The dialogue was conducted without a dedicated agentic system, custom
  orchestration, systematic evaluator, problem-specific search engine,
  or formal proof assistant set up in advance by the author. We also
  record, as process tracing rather than as a causal claim, that the
  author's problem-independent meta-level interventions occurred near
...
\section{2. The degree/diameter problem and the main
result}\label{the-degreediameter-problem-and-the-main-result}

\subsection{2.1 Basic graph-theoretic terminology and the
degree/diameter
problem}\label{basic-graph-theoretic-terminology-and-the-degreediameter-problem}

The graphs considered in this paper consist of finitely many vertices
and edges joining pairs of vertices. A graph whose edges are oriented is
...
For integers \(\Delta,D\ge1\), define \[
  N(\Delta,D)
\] to be the maximum number of vertices in a simple undirected connected
...
that any two vertices are joined by a path of length at most \(5\). For
example, constructing a \(12\)-regular graph of diameter \(5\) means
explicitly giving a large graph in which every vertex has degree \(12\)
and every pair of vertices is joined within \(5\) steps.

\subsection{2.2 Known lower bounds and the improvement in this
paper}\label{known-lower-bounds-and-the-improvement-in-this-paper}

Comellas's Mendeley Data V11 collects the largest known
\((\Delta,D)\)-graphs as of January 2026 {[}Comellas2026,
ComellasData2026{]}. In the related web table, checked on 31 May 2026,
the entries relevant to this paper had the same lower-bound values. The
known lower bounds for diameter \(5\) relevant to this paper are as
follows.
...
\textbf{Theorem 2.1.} There exist explicit simple undirected graphs
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1429}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.3117}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1429}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1429}}@{}}
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\((12,5)\) & \(29{,}621\) & \(34{,}992\) & \(+5{,}371\) (\(+18.13\%\)) &
\(193{,}261\) & \(18.11\%\) \\
\midrule
\((16,5)\) & \(132{,}496\) & \(147{,}456\) & \(+14{,}960\)
(\(+11.29\%\)) & \(867{,}857\) & \(16.99\%\) \\
\end{longtable}
}
...
\subsection{2.3 Outline of the construction: controller and
...
\textbf{Proposition 2.2 (route-chart lift principle).} Let \(C\) be a
...
route chart on \(\mathbb F_q^s\) is given, meaning that the
controllability matrix is nonsingular for every reduced word of length
\(s\). If \(C\) is an exact-NB-\(s\) controller, then this data defines
a simple \(\rho q\)-regular graph \(G\) on \(V(C)\times\mathbb F_q^s\),
and \[
  |V(G)|=|V(C)|q^s,
  \qquad
...
\bottomrule\noalign{}
\endlastfoot
controller & a small graph determining the coarse routing of the final
graph & every pair of controller vertices is joined by a length-\(5\)
nonbacktracking walk \\
\midrule
fiber chart & linear-algebraic data moving fiber coordinates along
controller routes & \(M_w\) is nonsingular for every reduced word of
length \(5\) \\
\end{longtable}
}
...
Appendix A.5, together with a voltage assignment with values in \[
  H=C_4\times C_6,
\] we obtain a \(144\)-vertex simple \(4\)-regular connected graph \(C\)
by the standard voltage lift defined in Appendix A.2. For this \(C\),
...
has diameter exactly \(5\), and Theorem 2.1 follows.

The construction is closely related to the existing theory of graph
lifts and voltage graphs. The controller in §A.5 is an ordinary
...
construction and existing methods is discussed in §7.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{3. Dialogue environment}\label{dialogue-environment}

The search was conducted by the author through ordinary dialogue in the
standard ChatGPT web interface. The author did not use proprietary
external orchestration, a custom agent framework, a problem-specific
search engine prepared in advance, or a formal proof assistant. The
...
§4--§5 are the author's English translations; the original Japanese
messages are preserved in the supplementary visible transcript (Appendix
B).

Because the ChatGPT account appeared to have access to information from
other sessions in the same account, this paper does not claim that the
main transcript released here was, by itself, the sole source of
information for the search. To the extent verified by the author, the
...
\item
  ...
  shown in §2. The released main session was the first to reach the
  present result.
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{4. The discovery process}\label{the-discovery-process}

The observations below concern Rounds 001--052 of the ChatGPT sessions,
...
\subsection{4.1 The initial prompt}\label{the-initial-prompt}
...
\subsection{4.2 Initial exploration}\label{initial-exploration}
...
\subsection{4.3 The first essential transition: route as coordinate
...
core of the final construction outlined in §2.3. In Rounds 022 and 023,
...
voltage-lift point of view. In an ordinary voltage lift, one attaches a
...
a finite list of fiber-side linear-algebraic conditions, followed by
their composition through a linear-algebra lemma.

\subsection{4.4 Success of the fiber chart and the controller
...
\subsection{4.5 Transition to diameter 5 and the first complete
...
\subsection{4.6 Post-completion generalization search and the update to
the final
...
\section{5. Transcript-based observations on the discovery
...
\subsection{5.1 Record of unsuccessful approaches and changes of
...
structured candidate graphs (roughly Rounds 004--015), the direction
that the LLM tentatively called the ``Hashimoto--de Bruijn packet
construction'' (Rounds 017--018), and the post-completion generalization
search (around Rounds 040--046). None of these unsuccessful searches is
...
that could be removed by a minor adjustment. This observation is
recorded only as transcript-level background preceding the later
emphasis on explicit covering identities and finite certificate-like
conditions.
...
In Rounds 040--046, several universal charts were found, but no
corresponding controller was found. In Round 047, the LLM stated that
one should separate the route-chart principle itself from the ad hoc
search for a controller, and withdraw from the latter. In the subsequent
Rounds 049--050, the final \(144\)-vertex controller was obtained.
...
\subsection{5.2 Constraints contained in the initial
...
known constructions. As noted in §2.3, however, each element of the
...
problem. This point is discussed in detail in §7.

\subsection{5.3 The user's meta-level interventions and representative
exchanges}\label{the-users-meta-level-interventions-and-representative-exchanges}

In the visible transcript, the author's general, problem-independent
interventions occurred near several junctures at which the level of
abstraction of the search changed. The interventions referred to here
did not supply components of the final construction. They were general
instructions about how to proceed with the search, when to stop, novelty
requirements, and requests to explain the prospects of success in
advance.
...
for which there is a mathematical reason to expect success. All are
general research instructions transferable to other search problems.
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1121}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3084}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3551}}
...
\bottomrule\noalign{}
\endlastfoot
001 & ``an idea not essentially contained in the literature''; ``do not
use web search, in principle'' & After surveying the table of known
records, states that it will try a construction that is not an extension
of existing methods & Gave general constraints on the search as an
initial condition \\
\midrule
003 & ``I do not feel that a good graph can be obtained by local
improvement'' & Acknowledges that random lift repair is weak as the main
strategy for a large new record, and switches to a construction whose
diameter proof closes without repair & Appears to have curbed the drift
toward local repair \\
\midrule
017 & also prohibits ``simply trying constructions from methods or
concepts a mathematician would think of immediately'' & States that it
will ``rebuild the construction principle itself, rather than improving
candidates,'' and makes the waste of the distance itself the object of
design & A point at which the novelty constraint was explicitly
strengthened \\
\midrule
022 & requests concentration on ``ideas with a reason to expect
efficient connectivity in advance'' & Discards the ``projective
rank-controller direction'' as lacking an a priori basis, and says that
a new principle with an explicit covering identity is needed & A point
at which the evaluation axis moved from mere near misses to a provable
covering principle \\
\midrule
023 & ``please devise a new principle'' & Proposes the controllable
route-chart lift, explaining that a single route together with \(q^5\)
controls covers the whole fiber & The point at which the abstraction
underlying the final construction was first made explicit \\
\midrule
026 & ``For the current 58-vertex controller problem, existing ideas may
be used, provided that there is a mathematical reason to expect
success.'' & Treats existing voltage-lift ideas as admissible for the
isolated controller subproblem, but only under an a-priori-success
criterion & Clarified the boundary between the novelty constraint and
the controlled use of existing tools \\
\midrule
036 & ``please continue'' & From a \(132\)-vertex exact-NB-5 controller
and an \(\mathbb F_4^5\) chart, obtains the first complete candidate
\((16,5,135{,}168)\) & A cautionary example showing that large progress
can occur even after a simple continuation instruction \\
\midrule
047 & ``please pause and review the direction'' & Summarizes that the
generalization search is drifting toward trial-and-error controller
search, and proposes a ``sumset-certified controller'' & A point at
which the search after the first complete candidate was re-abstracted \\
\midrule
049--050 & A renewed request in Round 049 to avoid ``methods with no
mathematical reason to expect success,'' followed by a brief
continuation instruction in Round 050 & Focuses the controller search on
a \(C_4\times C_6\)-based voltage-lift design, and in Round 050 obtains
the final \(144\)-vertex exact-NB-5 controller, yielding the
...
unit of progress. The analysis treats Round 023, Round 036, and Rounds
...
nor that the present method is generally effective. Evaluating success
...
generated solely by the LLM used in the session analyzed here. It was
prepared by the author from files generated in the observed session,
together with later LLM-assisted drafts produced using Claude Code Opus
4.7, Claude Code Opus 4.8, and ChatGPT Pro, and was then edited by the
author. The process analyzed in this paper is the process leading to the
construction of the solution, including the generation of the concrete
graph edge lists included in the supplementary materials. It does not
...
\subsection{7.1 Novelty of the graph construction
...
widely used {[}BrankovicEtAl1998a, LozSiran2008{]}. The controller
...
control-indexed parallel edges {[}GrossTucker2001, DalfoEtAl2021{]}.
...
\subsection{7.2 Decomposition of search execution and search
...
\subsection{7.3 Relation to recent
cases}\label{relation-to-recent-cases}

Apart from the system-oriented methods listed in §1, there have recently
...
orchestration {[}Bubeck2025, Salim2025, Verbeken2026, Gowers2026,
Howlett2026{]}. In particular, {[}Verbeken2026{]} is a single-case study
...
result was reached {[}Salim2025, Verbeken2026{]}. Others explicitly
...
{[}Gowers2026{]}. The materials released in {[}Gowers2026{]}, however,
consist mainly of artifacts and a narrative record, rather than a long
verbatim transcript of the kind released with this paper. Moreover, that
case is described as a relatively short exchange on the order of
minutes, so the time scale of the record differs from the \(52\)-round
...
contribution was concentrated on general control not specific to the
degree/diameter problem. This control included demanding novelty,
stopping and re-abstracting, and requesting explanations of the
prospects of success in advance. It did not consist in supplying
concrete components of the construction (§4, §5).

The difference from {[}Gowers2026{]} lies instead in the form and time
...
Thus, beyond the absence of problem-specific proof or construction hints
...
{[}RomeraParedes2024{]} or AlphaProof {[}Hubert2026{]}. It also remains
...
verbalize lessons drawn from unsuccessful approaches. It also contains
...
\section{Appendix A. Mathematical construction and
...
\subsection{A.1 Multigraphs and nonbacktracking
...
\textbf{Definition A.1.} A \emph{loopless multigraph} is a triple
...
\textbf{Definition A.2.} A walk of length \(\ell\) in
...
\subsection{A.2 Voltage lifts and the
...
group {[}GrossTucker2001{]}. In the degree/diameter problem too, the
...
in the base graph and net voltages, has been used {[}BrankovicEtAl1998a,
LozSiran2008{]}.
...
\textbf{Definition A.3.} Let \(H\) be a finite abelian group. An
...
\textbf{Lemma A.4.} Let \((B,\gamma)\) be an \(H\)-valued
...
\textbf{Proof.} Write \(c=(i,h_1)\), \(d=(j,h_2)\). Applying (A.0) to
...
By Petersen's 2-factor theorem {[}Petersen1891{]}, an even-regular
...
\textbf{Definition A.7.} Let \(C\) be a \(\rho\)-regular simple
...
\textbf{Lemma A.8.} With the above notation, suppose the route chart is
...
\item
  \(G\) is \(\rho q\)-regular.
\item
  If \(C\) is an exact-NB-\(s\) controller then
  \(\operatorname{diam}(G)\le s\).
\item
  \(|V(G)|=|V(C)|q^s\).
\end{enumerate}

\textbf{Proof.} The graph is undirected by Definition A.7. We first
...
yields the candidate neighbor \[
  (v,A_tx+\lambda b_t) .
\] By the argument at the end of Definition A.7, these are all the
...
from \((c,x)\) to \((d,y)\). Hence \(\operatorname{diam}(G)\le s\). The
vertex count follows immediately from the definition. \(\square\)

\textbf{Remark.} The route-chart lift of Definition A.7 can also be
...
graph lift by a permutation voltage assignment {[}GrossTucker2001,
...
\] Each edge is as in Table A.1, with the orientation \(u\to v\) in the
table taken as the forward direction.

\textbf{Table A.1.} Base multigraph \(B\) and voltage assignment.
...
Each vertex occurs four times in Table A.1, and there are no loops.
Hence \(B\) is a loopless \(4\)-regular multigraph. Setting
\(C:=B^\gamma\), \[
  V(C)=\{0,1,2,3,4,5\}\times C_4\times C_6 ,
\] so \(|V(C)|=6\cdot4\cdot6=144\).

\textbf{Proposition A.9.} \(C\) is a simple \(4\)-regular connected
graph and an exact-NB-5 controller.

\textbf{Proof.} Regularity follows from the definition of the regular
...
The base multigraph \(B\) is connected by inspection of Table A.1.
...
\] is \(-(1,1)+(1,2)+(1,5)=(1,0)\), which generates the \(C_4\)
component. Hence \(C\) is connected.

For the exact-NB-5 property, by Lemma A.4 it suffices to verify that,
...
most \(6\cdot324=1944\), so a complete enumeration is possible. The
supplementary \texttt{scripts/*/verify\_candidate.py} enumerates these
by DFS and verifies that all \(36\) ordered base pairs and all \(24\)
voltage classes are covered. \(\square\)

\subsection{\texorpdfstring{A.6 Universal route charts for
...
\subsubsection{\texorpdfstring{A.6.1 Chart on
...
\textbf{Proposition A.10.} The correspondence \((A_t,b_t)_{t\in S}\) is
a universal route chart on \(\mathbb F_3^5\).

\textbf{Proof.} The supplementary
...
These are exactly the conditions of Definition A.6 and universality.
\(\square\)

\subsubsection{\texorpdfstring{A.6.2 Chart on
...
\textbf{Proposition A.11.} The correspondence \((A_t,b_t)_{t\in S}\) is
...
kinds of conditions as in Proposition A.10. That is, the inverse
relations (A.1) and the rank-\(5\) condition for all \(324\)
controllability matrices hold. \(\square\)

\subsection{A.7 Proof of the main
theorem}\label{a.7-proof-of-the-main-theorem}

\textbf{Theorem A.12.} The following inequalities hold: \[
  N(12,5)\ge34{,}992,
  \qquad
  N(16,5)\ge147{,}456 .
\]

\textbf{Proof.} Fix the \(144\)-vertex controller \(C\) of §A.5 and the
inverse-consistent labeling \(\ell\) of §A.3.

First let \(q=3\) and use the universal route chart of §A.6.1. By
Proposition A.9, \(C\) is exact-NB-5, and by Proposition A.10 the chart
is universal. By Lemma A.8, \[
  G_3=G(C,S,\ell,A,b)
\] is simple \(4q=12\)-regular with \(\operatorname{diam}(G_3)\le5\),
and satisfies \[
...
§A.6.2, by Lemma A.8, \[
  G_4=G(C,S,\ell,A,b)
\] is simple \(16\)-regular with \(\operatorname{diam}(G_4)\le5\), and
satisfies \[
...
\(4\), and in both cases the diameter is exactly \(5\). \(\square\)

\subsection{A.8 Verification and
...
verifying finitely many conditions. The supplementary materials preserve
the construction package produced by the LLM during the dialogue as
\texttt{route\_chart\_verified\_graphs\_package/}. This directory is a
frozen provenance artifact, not a cleaned software release. It preserves
the process by which the construction was obtained.

Some generating scripts therefore contain unstandardized elements. These
include default output paths from the execution environment used during
the dialogue and minor differences in the format of the edge-list
header. These elements are not part of the mathematical claims of this
paper.

The objects used in the mathematical proof are the construction data
made explicit in Appendix A and the corresponding finite list of
certificate conditions. The natural-language description files inside
\texttt{route\_chart\_verified\_graphs\_package/} are kept as
provenance, but the proof is based on the definitions and lemmas of
Appendix A and on the verification procedures below.

This package has two parameter directories, \texttt{12\_5\_34992} and
\texttt{16\_5\_147456}, and mainly contains the following.

\begin{itemize}
\tightlist
\item
  \texttt{construction\_details.md}: the LLM-generated explanation of
  the route-chart lift construction.
\item
  \texttt{certificates/*/controller\_c4xc6\_multibase.txt}: the base
  data of Table A.1.
\item
  \texttt{certificates/12\_5\_34992/gf3\_s5\_universal.txt} and
  \texttt{certificates/16\_5\_147456/gf4\_s5\_universal.txt}: the chart
  data of §A.6.
\item
  \texttt{scripts/*/verify\_candidate.py}: a script that verifies the
  finite conditions of base voltage coverage,
  simplicity/regularity/connectivity of the controller, the labeling,
  and chart universality.
\item
  \texttt{scripts/*/generate\_final\_graph.py}: a script that generates
  the edge list.
\item
  \texttt{graphs/*/final\_graph\_edges.tsv.gz}: the generated edge list.
\item
  \texttt{SHA256SUMS.txt}: SHA-256 checksums of the main files inside
  \texttt{route\_chart\_verified\_graphs\_package/}.
\end{itemize}

\texttt{certificates/*/original\_README.txt} and
\texttt{certificates/12\_5\_34992/original\_phase35\_report.md} are kept
as provenance for explanations produced by the LLM during the process
leading to the construction. These descriptions are not a substitute for
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Certificate-level verification.} Each
  \texttt{scripts/*/verify\_candidate.py} verifies the voltage coverage
  of length-\(5\) nonbacktracking walks on the base multigraph, the
  simplicity / \(4\)-regularity / connectivity of the regular lift
  controller, the inverse-consistent labeling, and the rank-\(5\)
  condition of the controllability matrix for all \(324\) reduced words
  of length \(5\). The expected output has the following form.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Base voltage coverage over C4 x C6: OK}
\NormalTok{Controller: OK; 144 vertices, simple 4{-}regular, exact labeled NB length{-}5 coverage}
\NormalTok{GF(q)\^{}5 universal chart: OK; 324 reduced words checked, min rank 5; inverse{-}symbol data verified}
\NormalTok{Certificate complete: degree 4q, diameter \textless{}=5, vertices = 144*q\^{}5}
\end{Highlighting}
\end{Shaded}
\item
  \textbf{Generation-level identity check.}
  \texttt{scripts/*/generate\_final\_graph.py} generates the final edge
  list. To preserve provenance, these generating scripts are, in
  principle, kept in the form obtained during the dialogue. Thus, at run
  time, one should make a working copy and explicitly specify the output
  path for the generated edge list before comparing it with the bundled
  file. The top-level \texttt{verify\_graphs\_identical.py} in the
  supplementary materials verifies that the graph obtained from the
  generating script and the graph bundled in
  \texttt{route\_chart\_verified\_graphs\_package/graphs/*/final\_graph\_edges.tsv.gz}
  are identical. The check has two parts. First, it compares the SHA-256
  hash of the edge rows, excluding the header. Second, it checks
  agreement of the edge sets after normalizing the orientation of each
  edge. The first part verifies agreement of the edge rows in the same
  order. The second verifies graph-theoretic identity independently of
  file format or row order.
\item
  \textbf{Graph-level independent check.}
  \texttt{verify\_with\_networkx.py} is an optional independent
  verification script that directly reads the bundled edge list and
  checks the absence of self-loops, the absence of multiple edges, the
  number of vertices, the number of edges, the regular degree, the
  handshake equality, and the exact diameter computed by NetworkX. This
  script does not depend on the natural-language descriptions or the
  generating scripts inside the LLM-generated package, and uses the edge
  list itself as its target. Note that the exact diameter computation is
  an additional check that may take time for large graphs, and the
  diameter-\(5\) proof of this paper does not depend on this script.
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Appendix B. Structure of the supplementary
materials}\label{appendix-b.-structure-of-the-supplementary-materials}

The supplementary materials contain four types of files. These are the
visible transcript of the discovery process, the script that converts
the conversation log into the visible transcript, the construction
package produced by the LLM and preserved as provenance, and the scripts
for collation and independent verification.
...
\item
  ...
  only the messages that appear in the browser UI as ordinary user
  prompt bubbles or assistant answer bubbles. Chain-of-thought, tool
  calls, tool outputs, reasoning recaps, and system messages are
  excluded.
\item
  \texttt{extracted/DDP\_visible\_transcript.md}: the visible transcript
  extracted by \texttt{dump\_ddp\_visible.py}. It targets Rounds
  001--052 relevant to the present discovery and verification.
\item
  \texttt{route\_chart\_verified\_graphs\_package/}: the construction
  package produced by the LLM during the dialogue, included, in
  principle, as raw files for provenance preservation. Details are given
  in A.8 of Appendix A.
\item
  \texttt{verify\_graphs\_identical.py}: a script that verifies that the
  graph generated from \texttt{generate\_final\_graph.py} and the graph
  bundled under \texttt{route\_chart\_verified\_graphs\_package/graphs/}
  are identical. It checks the SHA-256 of the edge rows excluding the
  header, and the agreement of the normalized edge set.
\item
  \texttt{verify\_with\_networkx.py}: an independent verification
  script. It checks the bundled graph for the absence of self-loops, the
  absence of multiple edges, the number of vertices, the number of
  edges, the regular degree, and the handshake equality, and also
  computes the exact diameter with NetworkX. The diameter computation of
  this script is an optional additional check, and the diameter-\(5\)
  proof of this paper does not depend on this script.
\end{itemize}
...
{[}AlonEtAl2026{]} N. Alon, T. F. Bloom, W. T. Gowers, D. Litt, W.
Sawin, A. Shankar, J. Tsimerman, V. Wang, and M. Matchett Wood. Remarks
on the disproof of the unit distance conjecture. arXiv:2605.20695, 2026.
DOI: 10.48550/arXiv.2605.20695.

{[}Bannai1973{]} E. Bannai and T. Ito. On finite Moore graphs. Journal
of the Faculty of Science, University of Tokyo, Section IA, Mathematics
20 (1973), 191--208.

{[}BermondDelormeFarhi1984{]} J.-C. Bermond, C. Delorme, and G. Farhi.
Large graphs with given degree and diameter II. \emph{Journal of
Combinatorial Theory, Series B} 36 (1984), 32--48. DOI:
10.1016/0095-8956(84)90012-1.

{[}BermondDelormeQuisquater1986{]} J.-C. Bermond, C. Delorme, and J.-J.
Quisquater. Strategies for interconnection networks: some methods from
graph theory. \emph{Journal of Parallel and Distributed Computing} 3
(1986), 433--449. DOI: 10.1016/0743-7315(86)90008-0.

{[}BhanNobiliLanger2026{]} J. Bhan, N. Nobili, and P. Langer. New bounds
for Zarankiewicz numbers via reinforced LLM evolutionary search.
arXiv:2605.01120, 2026. DOI: 10.48550/arXiv.2605.01120.

{[}BrankovicEtAl1998a{]} L. Branković, M. Miller, J. Plesník, J. Ryan,
and J. Širáň. Large graphs with small degree and diameter: A voltage
assignment approach. Australasian Journal of Combinatorics 18 (1998),
65--76.

{[}Bubeck2025{]} S. Bubeck, C. Coester, R. Eldan, T. Gowers, Y. T. Lee,
A. Lupsasca, M. Sawhney, R. Scherrer, M. Sellke, B. K. Spears, D.
Unutmaz, K. Weil, S. Yin, and N. Zhivotovskiy. Early science
acceleration experiments with GPT-5. arXiv:2511.16072, 2025. DOI:
10.48550/arXiv.2511.16072.

{[}Comellas2026{]} F. Comellas. Table of large graphs with given degree
and diameter. arXiv:2406.18994v2, 2026. DOI:
10.48550/arXiv.2406.18994v2.

{[}ComellasData2026{]} F. Comellas. Table of Large Degree/Diameter
Graphs. Mendeley Data, Version 11, published 28 January 2026. DOI:
10.17632/d75dzbjd4k.11.

{[}DalfoEtAl2021{]} C. Dalfó, M. A. Fiol, S. Pavlíková, and J. Širáň.
Spectra and eigenspaces of arbitrary lifts of graphs. Journal of
Algebraic Combinatorics 54 (2021), 651--672. DOI:
10.1007/s10801-021-01049-3.

{[}DelormeBipartite1985{]} C. Delorme. Large bipartite graphs with given
degree and diameter. \emph{Journal of Graph Theory} 9(3) (1985),
325--334. DOI: 10.1002/jgt.3190090304.

{[}Damerell1973{]} R. M. Damerell. On Moore graphs. Mathematical
Proceedings of the Cambridge Philosophical Society 74(2) (1973),
227--236. DOI: 10.1017/S0305004100048015.

{[}Gowers2026{]} T. Gowers. A recent experience with ChatGPT 5.5 Pro.
Gowers's Weblog, 8 May 2026.
\url{https://gowers.wordpress.com/2026/05/08/a-recent-experience-with-chatgpt-5-5-pro/}

{[}GrossTucker2001{]} J. L. Gross and T. W. Tucker. \emph{Topological
Graph Theory}. Dover, 2001.

{[}HoffmanSingleton1960{]} A. J. Hoffman and R. R. Singleton. On Moore
graphs with diameters two and three. \emph{IBM Journal of Research and
Development} 4(5) (1960), 497--504. DOI: 10.1147/rd.45.0497.

{[}Howlett2026{]} J. Howlett. An amateur just solved a 60-year-old math
problem---by asking AI. Scientific American, 24 April 2026.

{[}Hubert2026{]} T. Hubert, R. Mehta, L. Sartran, M. Z. Horváth, G.
Žužić, E. Wieser, A. Huang, J. Schrittwieser, Y. Schroecker, H. Masoom,
et al.~Olympiad-level formal mathematical reasoning with reinforcement
learning. \emph{Nature} 651 (2026), 607--613. DOI:
10.1038/s41586-025-09833-y.

{[}JuEtAl2026{]} H. Ju, G. Gao, J. Jiang, B. Wu, Z. Sun, S. Liu, L.
Chen, Y. Wang, Y. Wang, Z. Wang, W. He, P. Wu, L. Xiao, R. Liu, B. Dai,
and B. Dong. Automated conjecture resolution with formal verification.
arXiv:2604.03789, 2026. DOI: 10.48550/arXiv.2604.03789.

{[}LozSiran2008{]} E. Loz and J. Širáň. New record graphs in the
degree-diameter problem. \emph{Australasian Journal of Combinatorics} 41
(2008), 63--80.

{[}MillerSiran2013{]} M. Miller and J. Širáň. Moore graphs and beyond: A
survey of the degree/diameter problem. \emph{Electronic Journal of
Combinatorics}, Dynamic Survey DS14, 2nd edition, 2013. DOI:
10.37236/35.

{[}NagdaRaghavanThakurta2026{]} A. Nagda, P. Raghavan, and A. Thakurta.
Reinforced generation of combinatorial structures: Ramsey numbers.
arXiv:2603.09172, 2026. DOI: 10.48550/arXiv.2603.09172.

{[}OpenAI2026{]} OpenAI. An OpenAI model has disproved a central
conjecture in discrete geometry. 2026.
\url{https://openai.com/index/model-disproves-discrete-geometry-conjecture/}

{[}Petersen1891{]} J. Petersen. Die Theorie der regulären Graphs. Acta
Mathematica 15 (1891), 193--220.

{[}PrzybockiMackeyHeuleSubercaseaux2026{]} B. Przybocki, J. Mackey, M.
J. H. Heule, and B. Subercaseaux. Doubly saturated Ramsey graphs: A case
study in computer-assisted mathematical discovery. arXiv:2604.21187,
2026. DOI: 10.48550/arXiv.2604.21187.

{[}RomeraParedes2024{]} B. Romera-Paredes, M. Barekatain, A. Novikov, M.
Balog, M. P. Kumar, E. Dupont, F. J. R. Ruiz, J. S. Ellenberg, P. Wang,
O. Fawzi, P. Kohli, and A. Fawzi. Mathematical discoveries from program
search with large language models. \emph{Nature} 625 (2024), 468--475.
DOI: 10.1038/s41586-023-06924-6.

{[}Salim2025{]} A. Salim. Accelerating mathematical research with
language models: A case study of an interaction with GPT-5-Pro on a
convex analysis problem. arXiv:2510.26647, 2025. DOI:
10.48550/arXiv.2510.26647.

{[}Trinh2024{]} T. H. Trinh, Y. Wu, Q. V. Le, H. He, and T. Luong.
Solving olympiad geometry without human demonstrations. \emph{Nature}
625 (2024), 476--482. DOI: 10.1038/s41586-023-06747-5.

{[}TsoukalasEtAl2026{]} G. Tsoukalas, A. Kovsharov, S. Shirobokov, A.
Surina, M. Firsching, G. Bérczi, F. J. R. Ruiz, A. Suggala, A. Z.
Wagner, E. Wieser, L. Yu, A. Huang, M. Z. Horváth, A. Ferrauiolo, H.
Michalewski, C. Grosu, T. Hubert, M. Balog, P. Kohli, and S. Chaudhuri.
Advancing mathematics research with AI-driven formal proof search.
arXiv:2605.22763, 2026. DOI: 10.48550/arXiv.2605.22763.

{[}Verbeken2026{]} B. Verbeken, B. Vagenende, M.-A. Guerry, A. Algaba,
and V. Ginis. Early Evidence of Vibe-Proving with Consumer LLMs: A Case
Study on Spectral Region Characterization with ChatGPT-5.2 (Thinking).
arXiv:2602.18918, 2026. DOI: 10.48550/arXiv.2602.18918.

{[}Woodruff2026{]} D. P. Woodruff, V. Cohen-Addad, L. Jain, J. Mao, S.
Zuo, M.-H. Bateni, S. Branzei, M. P. Brenner, L. Chen, Y. Feng, L.
Fortnow, G. Fu, Z. Guan, Z. Hadizadeh, M. T. Hajiaghayi, et
al.~Accelerating scientific research with Gemini: Case studies and
common techniques. arXiv:2602.03837, 2026. DOI:
10.48550/arXiv.2602.03837.

{[}Zhang2026{]} C. Zhang, J. Song, S. Li, Y. Liang, Y. Ma, W. Wang, Y.
Zhu, and S.-C. Zhu. Proposing and solving olympiad geometry with guided
tree search. \emph{Nature Machine Intelligence} 8 (2026), 84--95. DOI:
10.1038/s42256-025-01164-x.
