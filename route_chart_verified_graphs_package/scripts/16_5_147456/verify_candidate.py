...
MUL=[[gf_mul(i,j) for j in range(4)] for i in range(4)]; INV=[None,1,3,2]
A=[[[2,0,1,2,0],[1,2,2,0,0],[3,0,0,3,0],[0,0,1,3,0],[2,1,2,2,2]],[[2,0,3,2,0],[0,3,1,1,0],[1,0,3,0,0],[2,0,1,2,0],[1,2,2,3,3]],[[3,1,3,2,0],[0,1,0,0,0],[1,0,2,0,0],[2,1,0,0,0],[3,2,2,0,2]],[[0,3,0,3,0],[0,1,0,0,0],[0,2,3,2,0],[3,1,1,2,0],[0,2,3,3,3]]]
B=[[2,1,2,0,2],[2,1,3,1,2],[2,1,0,2,2],[2,1,1,3,2]]; SINV={0:1,1:0,2:3,3:2}
def matmul(X,Y):
    return [[sum([]) if False else _dotrowcol(X[i],Y,j) for j in range(len(Y[0]))] for i in range(len(X))]
def _dotrowcol(row,Y,j):
    z=0
    for k,a in enumerate(row): z ^= MUL[a][Y[k][j]]
    return z
def mv(M,v):
    z=[]
    for row in M:
        s=0
        for a,b in zip(row,v): s ^= MUL[a][b]
        z.append(s)
    return z
def rank_cols(cols):
    M=[list(r) for r in zip(*cols)]; r=0
    for c in range(len(M[0])):
        p=next((i for i in range(r,len(M)) if M[i][c]),None)
        if p is None: continue
        M[r],M[p]=M[p],M[r]; inv=INV[M[r][c]]; M[r]=[MUL[inv][x] for x in M[r]]
        for i in range(len(M)):
            if i!=r and M[i][c]:
                f=M[i][c]; M[i]=[M[i][j]^MUL[f][M[r][j]] for j in range(len(M[0]))]
        r+=1
        if r==len(M): break
    return r
def verify_chart():
    ...
    gen([]); mr=99
    for w in words:
        suff=[[1 if i==j else 0 for j in range(5)] for i in range(5)]; cols=[]
        for s in reversed(w): cols.append(mv(suff,B[s])); suff=matmul(suff,A[s])
...
if __name__=='__main__':
    ...
    print(f'GF(4)^5 universal chart: OK; {nw} reduced words checked, min rank {mr}; inverse-symbol data verified')
