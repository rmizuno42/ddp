...
where <package> = route_chart_verified_graphs_package.
...
Each .tsv.gz file consists of:
    - line 1: header  (the (12,5) generator writes "# n=... m=... degree=12";
              the (16,5) generator and the bundled files write "n m")
...
Usage:
    python3 verify_graphs_identical.py \\
        --package /path/to/route_chart_verified_graphs_package \\
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Compare user-generated graphs against the bundled graphs "
                    "in route_chart_verified_graphs_package (read-only).")
    ap.add_argument(
        "--package", type=Path, required=True,
        help="Path to the route_chart_verified_graphs_package directory "
